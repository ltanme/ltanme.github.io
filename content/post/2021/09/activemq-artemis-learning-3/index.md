---
title: "Activemq Artemis Learning 3"
date: 2021-09-04T17:16:44+08:00
draft: false
tags: ["artemis","maven","stomp","mqtt","netty"]
---
...
>
> 这里说一下为什么需要重复，反复执行mvn -Prealse install ,因为项目的依赖都来自不同的网络，其主要的需要从maven 全球中央仓库下载，有时会因为网络掉线，访问服务器节点超时引起的编译程序中断导致，所以重复执行mvn 打包命令。是可以理解的
>
> 说到这里有人可能会问，为什么不用`github` 自带的`workflow `编译，因为那是国外环境，
>
> 当然没有墙的问题，在本地编译主要目的还是为了检验源代码完整性以及 后面二次开发扩展打基础。
>
> 以下是我编译的最后几部过程的全结果。  最后项目编译共花了`43分钟` 包含测试用例，第一次成功编译后，后面编译就会有`cache`, 顺利多了，编译后最终的程序打包是在 `artemis-distribution` 模块下面
